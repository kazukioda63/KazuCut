...
### 3.3 Premiere編集
元シーケンス複製 → 複製へ編集適用。Aロール映像+内蔵音声のみ編集。
A/V同期維持、対象外トラック不変、編集後自動検証、失敗時は元シーケンス保護。
...
UI選択肢: 「複製シーケンスで編集」（初期値）/「元シーケンスを直接編集」（上級者向け）。
直接編集時も先にバックアップ複製作成。バックアップ失敗なら直接編集を開始しない。
...
### 適用前確認
```
対象：V1 / A1 / 候補：14件 / 推定短縮：9.8秒 / 出力：複製シーケンス
対象外トラック：変更しません
☑ 元シーケンスを保護して編集します
[選択した候補を適用]
```
